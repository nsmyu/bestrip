...
RSpec.describe "Posts", type: :request do
  ...
  describe "GET #index" do
    ...
    it "投稿を全て取得すること" do
      test_post_1 = create(:post, :with_photo, itinerary: itinerary)
      test_post_2 = create(:post, :with_photo, itinerary: itinerary)
      get posts_path
      expect(response.body).to include test_post_1.title, test_post_2.title
    end

    it "投稿のタイトル、投稿者名、投稿日を取得すること" do
      test_post = create(:post, :with_photo, itinerary: itinerary)
      get posts_path
  ...
  describe "GET #edit" do
    before do
      get edit_post_path(id: test_post.id)
    end
    ...
    it "投稿の各情報を取得すること" do
      expect(response.body).to include test_post.title
      expect(response.body).to include test_post.caption
      expect(response.body).to include test_post.itinerary.title
    end
  end
  ...
  describe "PATCH #update" do
    context "有効な値の場合" do
      it "各項目の変更に成功すること" do
        ...
        patch post_path(id: test_post.id), params: { post: post_params }
        expect(response).to redirect_to post_path(test_post.id)
        expect(test_post.reload.title).to eq "Edited Title"
        expect(test_post.reload.caption).to eq "Edited caption."
        expect(test_post.reload.itinerary_id).to eq other_itinerary.id
      end
    end
    ...
    context "無効な値の場合" do
      ...
      it "写真が1枚も選択されていない（既存の写真が削除される）場合、失敗すること" do
        photo_delete_params =
          { photos_attributes: [id: test_post.photos[0].id, _destroy: true] }
  ...
  describe "DELETE #destroy" do
    it "成功すること" do
      delete post_path(id: test_post.id)
      expect(response).to redirect_to posts_path
    end
