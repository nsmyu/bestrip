...
RSpec.describe User, type: :model do
  ...
  describe "子モデルのレコード削除" do
    ...
    it "userを削除すると関連するitineraryも削除されること" do
      create(:itinerary, owner: user)
      expect { user.destroy }.to change { Itinerary.count }.by(-1)
    end

    it "userを削除すると関連するplaceも削除されること" do
      create(:user_place, placeable: user)
      expect { user.destroy }.to change { Place.count }.by(-1)
    end

    it "userを削除すると関連するpending_invitationも削除されること" do
      create(:pending_invitation, user: user)
