...
RSpec.describe Schedule, type: :model do
  ...
  it "スケジュールの日付が出発日より前の場合は無効であること" do
    itinerary = create(:itinerary)
    schedule = build(:schedule, date: "2024-01-31", itinerary: itinerary)
    schedule.valid?
    expect(schedule.errors).to be_of_kind(:date, "出発日〜帰宅日の間で選択してください")
  end

  it "スケジュールの日付が帰宅日より後の場合は無効であること" do
    itinerary = create(:itinerary)
    schedule = build(:schedule, date: "2024-02-09", itinerary: itinerary)
    schedule.valid?
    expect(schedule.errors).to be_of_kind(:date, "出発日〜帰宅日の間で選択してください")
  end
  ...
  it "itinerary_idがなければ無効であること" do
    schedule = build(:schedule, date: nil, itinerary: nil)
    schedule.valid?
